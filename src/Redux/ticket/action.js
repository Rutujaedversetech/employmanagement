...
import {
  ...
  GET_EMP_TICKET_REQUEST,
  GET_EMP_TICKET_SUCCESS,
  GET_EMP_TICKET_FAILURE,
  ADD_TICKET_REQUEST,
  ADD_TICKET_SUCCESS,
  ADD_TICKET_FAILURE,
...
export const getempTicket = (id) => async (dispatch) => {
  console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_TICKET_REQUEST });
  ...
  //  const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id='Emp007`, {
   //   `http://localhost:8080/service_request/assets`,
      const res=  await fetch(`http://68.178.169.96:3000/service_request/ticket`, {
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_TICKET_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_TICKET_FAILURE,
    });
  }
};
...
export const addempticket = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
  ...
  try {
    ...
    const res = await axios.post(
      'http://68.178.169.96:3000/service_request/ticket'  ,    data
...
export const updateTicketStatus = (id, data) => async (dispatch) => {
  console.log('opp',data);
  try {
    dispatch({ type: UPDATE_TICKET_REQUEST });

    await fetch(`http://68.178.169.96:3000/service_request/${id}`, {
