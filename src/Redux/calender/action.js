...
import {
  ...
  GET_EMP_CALENDER_REQUEST,
  GET_EMP_CALENDER_SUCCESS,
  GET_EMP_CALENDER_FAILURE,
  ADD_CALENDER_REQUEST,
  ADD_CALENDER_SUCCESS,
  ADD_CALENDER_FAILURE,
...
export const getempcalender = (id) => async (dispatch) => {
  //console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_CALENDER_REQUEST,payload:"loading" });
  ...
  //  const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id='Emp007`, {
   //   `http://localhost:8080/service_request/assets`,
      const res=  await fetch(`http://68.178.169.96:3000/calendar/holiday_date_from?holiday_date_from=${id}`, {
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_CALENDER_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_CALENDER_FAILURE,
    });
  }
};
...
export const addempcalender = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
  console.log('data',data);
  try {
    dispatch({ type: ADD_CALENDER_REQUEST });
