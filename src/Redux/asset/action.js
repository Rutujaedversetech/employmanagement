...
import {
  ...
  GET_EMP_ASSET_REQUEST,
  GET_EMP_ASSET_SUCCESS,
  GET_EMP_ASSET_FAILURE,
  ADD_ASSET_REQUEST,
  ADD_ASSET_SUCCESS,
  ADD_ASSET_FAILURE,
...
export const getempasset = (id) => async (dispatch) => {
  console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_ASSET_REQUEST });
  ...
  //  const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id='Emp007`, {
   //   `http://localhost:8080/service_request/assets`,
      // const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id=${id}`, {
        const res=  await fetch(`http://68.178.169.96:3000/service_request/assets`, {

      // body: JSON.stringify(res),http://68.178.169.96:3000/service_request/ticket
      method: "GET",
      headers: {
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_ASSET_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_ASSET_FAILURE,
    });
  }
};
...
export const addempasset = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
...
export const updateASSETStatus = (id, data) => async (dispatch) => {
  console.log('opp',data);
  try {
    dispatch({ type: UPDATE_ASSET_REQUEST });

    await fetch(`http://68.178.169.96:3000/assetid/${id}`, {
    ...
    dispatch({ type: UPDATE_ASSET_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
