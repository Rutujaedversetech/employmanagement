...
export const getempallresign = (id) => async (dispatch) => {
  console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_RESIGN_REQUEST });
  ...
  //  const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id='Emp007`, {
   //   `http://localhost:8080/service_request/assets`,
      const res=  await fetch(`http://68.178.169.96:3000/job_posting`, {
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_RESIGN_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_RESIGN_FAILURE,
    });
  }
};
...
export const addempresign = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
  console.log('data',data);
  try {
    dispatch({ type: ADD_RESIGN_REQUEST });
...
export const updateRESIGNStatus = (id, data) => async (dispatch) => {
  console.log('opp',data);
  try {
    dispatch({ type: UPDATE_RESIGN_REQUEST });

    await fetch(`http://68.178.169.96:3000/job_posting/${id}`, {
    ...
    dispatch({ type: UPDATE_RESIGN_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: UPDATE_RESIGN_FAILURE,
    });
  }
};
...
export const deleteRESIGN = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_RESIGN_REQUEST });
    ...
    dispatch({ type: DELETE_RESIGN_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_RESIGN_FAILURE,
    });
  }
};
