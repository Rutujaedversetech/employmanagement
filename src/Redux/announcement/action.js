...
export const getempANNOUNCEMENT = (id) => async (dispatch) => {
  //console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_ANNOUNCEMENT_REQUEST ,payload:"loading"});
  ...
  //  const res=  await fetch(`http://68.178.169.96:3000/employee_asset/emp_id?emp_id='Emp007`, {
   //   `http://localhost:8080/service_request/assets`,
      // const res=  await fetch(`http://68.178.169.96:3000/calendar/anno_date_from?anno_date_from=${id}`, {
        const res=  await fetch(`http://68.178.169.96:3000/calendar`, {
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_ANNOUNCEMENT_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_ANNOUNCEMENT_FAILURE,
    });
  }
};
...
export const addempANNOUCEMENT = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
  ...
  try {
    ...
    const res = await axios.post(
      'http://68.178.169.96:3000/calendar'  ,    data
...
export const updateANNOUNCEMENTStatus = (id, data) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({ type: UPDATE_ANNOUNCEMENT_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
