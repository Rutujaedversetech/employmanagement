...
export const getempalljob = (id) => async (dispatch) => {
  console.log('is',id);
  //var id=id
    //console.log('is',id);
   // const token=localStorage.getItem("token")
  ...
  try {
    dispatch({ type: GET_EMP_JOB_REQUEST });
...
//console.log('fgh123',res.data);
    dispatch({ type: GET_EMP_JOB_SUCCESS, payload: data.response });
  } catch (error) {
    dispatch({
      type: GET_EMP_JOB_FAILURE,
    });
  }
};
...
export const addempjob = (data) => async (dispatch) => {
  // const headers = {
  //   token: localStorage.getItem("token"),
  // };
